...
public interface InvOnHandRepository extends CrudRepository<InvOnHand, StorerSkuPK> {
    ...
    @Query("SELECT inv FROM InvOnHand inv " +
            ...
            "AND inv.skuLotLocPK.sku NOT IN " +
            "(SELECT t.skuLotLocPK.sku FROM Tasks t WHERE t.status = 'Pending')")
    Iterable<InvOnHand> invWithCycleCount();

    @Query("SELECT inv FROM InvOnHand inv WHERE inv.skuLotLocPK.sku = :inputSku " +
            "AND inv.skuLotLocPK.loc NOT IN " +
            "(SELECT l.loc FROM Loc l WHERE l.locationFlag = 'Hold')")
    ArrayList<InvOnHand> invToCount(@Param("inputSku") String inputSku);
